...
st.title("🛡️ Scanner de Sécurité Réseau Pro")
st.markdown("""
...
if scan_mode == "Un seul appareil":
    ...
    if not re.match(r"^[0-9., ]+$", target_input) and target_input != "localhost":
        st.sidebar.warning("⚠️ Format d'IP invalide")
    target = target_input
else:
...
stealth_mode = st.sidebar.toggle("🕵️ Mode Furtif (Éviter le bannissement)", value=True)
...
if start_btn:
    ...
    if stealth_mode:
        st.warning("🕵️ Mode Furtif activé : Le scan sera beaucoup plus lent pour ne pas alerter le pare-feu.")
    
    # Zone de logs et de découverte
    st.subheader("🛰️ Chronologie de l'Audit")
    log_area = st.empty()
    log_content = []
                        ...
                        # Détection de vulnérabilités
                            ...
                            if "matched_regex" in line or "VULNERABILITY" in line.upper():
                                if module not in devices_status[ip]['vulns']:
                                    ...
                                    log_content.append(f"⚠️ Alerte : {module} sur {ip}")
